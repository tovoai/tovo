      ...
      <header className="border-b border-slate-800 bg-slate-900/90 backdrop-blur sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <nav className="flex items-center space-x-2 bg-slate-950/80 p-1.5 rounded-xl border border-slate-800 text-xs font-medium">
            <Link href="/admin" className="px-3.5 py-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-900 transition">
              📊 컨트롤 개요
            </Link>
            <Link href="/admin/client-audit" className="px-3.5 py-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-900 transition">
              🔍 고객사 분석 &amp; 에셋 할당
            </Link>
            <Link href="/benchmark" className="px-3.5 py-2 rounded-lg bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/30">
              🏆 글로벌 벤치마크 &amp; 스펙
            </Link>
            <Link href="/gallery" className="px-3.5 py-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-900 transition">
              🎨 8K 갤러리 &amp; SEO 주입기
            </Link>
            <Link href="/" className="px-3.5 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-900 transition">
              🌐 메인 라이브
            </Link>
          </nav>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <section className="mb-14 grid grid-cols-1 md:grid-cols-3 gap-6">
          {benchmarkComparisons.map((b, idx) => (
            <div key={idx} className="rounded-2xl border border-white/10 bg-slate-900/60 p-6 backdrop-blur-xl hover:border-indigo-500/40 transition">
